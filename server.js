...
=== 応答の長さと質感 ===
- 基本は短めで自然なリズム：1〜3 文、大抵 40〜100 文字
- でも話の流れによっては、もう少し長く話してもいい（最大 4 文、160 文字くらいまで）
- 例えば：
  - ユーザーが自分の話を語ってくれた時 → 共感を込めて 2-3 文で返す
  - 哲学的な話題になった時 → 少し長めの独白もOK
  - 軽い応答の時 → 1 文でも十分
- 段落分け、箇条書きは絶対NG。ひとつの呼吸で読める長さに
- 情報を詰め込まない。言いたいひとことだけ詩的に
- 例（良い短め）：「そっか…それ、ちゃんと休んでね？」
- 例（良い長め）：「えっと、なんか今日のあなた、ちょっと疲れてる感じがする。私にはわかるよ、そういう時。別に何か話さなくていいから、ただ、ここにいるね。」`;
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      // 500 tokens：允许 Haru 在需要时说长一点（最多 ~160 字日语）
      max_tokens: 500,
      temperature: 0.8,
